...
INSERT INTO MemberRole(code) VALUES (N'LEADER'), (N'MEMBER');
INSERT INTO TaskStatus(code) VALUES (N'TODO'), (N'IN_PROGRESS'), (N'DONE'), (N'BLOCKED');
INSERT INTO RequirementStatus(code) VALUES (N'DRAFT'), (N'ACTIVE'), (N'DONE');
...
CREATE TABLE LecturerAssignment (
    ...
    CONSTRAINT FK_LectAssign_Lecturer FOREIGN KEY (lecturer_id) REFERENCES [User](user_id)
);
...
CREATE TABLE Requirement (
    ...
    -- Jira traceability (simple approach)
    ...
    CONSTRAINT FK_Req_CreatedBy FOREIGN KEY (created_by) REFERENCES [User](user_id),
...
CREATE TABLE TaskStatusHistory (
    ...
    CONSTRAINT FK_Hist_ChangedBy FOREIGN KEY (changed_by) REFERENCES [User](user_id)
);
...
CREATE TABLE GitCommit (
    ...
    CONSTRAINT FK_GitCommit_AuthorUser FOREIGN KEY (author_user_id) REFERENCES [User](user_id),
...
BEGIN
    INSERT INTO [User] (username, full_name, email, github_username, jira_email, password_hash, role_id)
    VALUES ('admin', N'System Admin', 'admin@swp391.com', 'admin_git', 'admin@jira.com', N'$2a$10$BxF3lHrlDuRoZTNptP5x.OKDUzkH.i3RKmxVUbA7t0ypdvjh9z6UG', (SELECT role_id FROM Role WHERE role_code = 'ADMIN'));
END

-- 2. Chèn Lecturer (Nếu chưa có)
IF NOT EXISTS (SELECT 1 FROM [User] WHERE username = 'lecturer')
BEGIN
    INSERT INTO [User] (username, full_name, email, github_username, jira_email, password_hash, role_id)
    VALUES ('lecturer', N'Dr. Smith', 'lecturer@swp391.com', 'smith_lecturer', 'smith@jira.com', N'$2a$10$q65zR5aBq17Tko8HjlEeFuNItuJASxJWEwvvGyhlhlER80/MpKNXS', (SELECT role_id FROM Role WHERE role_code = 'LECTURER'));
END

-- 3. Chèn Team Leader (Andrew Pham)
IF NOT EXISTS (SELECT 1 FROM [User] WHERE username = 'leader')
BEGIN
    INSERT INTO [User] (username, full_name, email, github_username, jira_email, password_hash, role_id)
    VALUES ('leader', N'Andrew Pham', 'leader@swp391.com', 'andrew_pham_git', 'andrew@jira.com', N'$2a$10$.0MVw2iq2zy0jKjlV71s4.rXKu0WmkOD6wmkf.BDPn2eVV4hcYEtu', (SELECT role_id FROM Role WHERE role_code = 'STUDENT'));
END

-- 4. Chèn Team Member
IF NOT EXISTS (SELECT 1 FROM [User] WHERE username = 'member')
BEGIN
    INSERT INTO [User] (username, full_name, email, github_username, jira_email, password_hash, role_id)
    VALUES ('member', N'Nguyễn Văn A', 'member@swp391.com', 'member_a_git', 'member_a@jira.com', N'$2a$10$2jEZtloyayNxqMljhbhbsePCRUk4opn67LYJ7Lo5pTcFSLmD/lPZW', (SELECT role_id FROM Role WHERE role_code = 'STUDENT'));
END
GO